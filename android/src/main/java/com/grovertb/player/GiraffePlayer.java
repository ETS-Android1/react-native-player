...
import com.grovertb.player.ProxyPlayerListener;
...
public class GiraffePlayer implements MediaController.MediaPlayerControl {
    ...
    private GiraffePlayer(final Context context, final com.grovertb.player.VideoInfo videoInfo) {
        this.context = context.getApplicationContext();
        this.videoInfo = videoInfo;
        log("new GiraffePlayer");
        com.grovertb.player.VideoView videoView = PlayerManager.getInstance().getVideoView(videoInfo);
    ...
    private IMediaPlayer createMediaPlayer() {
        if (com.grovertb.player.VideoInfo.PLAYER_IMPL_SYSTEM.equals(videoInfo.getPlayerImpl())) {
    ...
    private void setOptions() {
        ...
        if (mediaPlayer instanceof IjkMediaPlayer) {
            ...
            for (com.grovertb.player.Option option : videoInfo.getOptions()) {
...
//                int videoSarDen = mp.getVideoSarDen();
    ...
    public static GiraffePlayer createPlayer(Context context, com.grovertb.player.VideoInfo videoInfo) {
    ...
    public GiraffePlayer setDisplayModel(int targetDisplayModel) {
        ...
        if (targetDisplayModel == DISPLAY_FULL_WINDOW) {
            ...
            com.grovertb.player.UIHelper uiHelper = com.grovertb.player.UIHelper.with(activity);
            ...
            final com.grovertb.player.VideoView videoView = PlayerManager.getInstance().getVideoView(videoInfo);
            if (videoView == null) {
                return this;
            }
            ...
            //change orientation & action bar
            com.grovertb.player.UIHelper uiHelper = UIHelper.with(activity);
            if (videoInfo.isPortraitWhenFullScreen()) {
                uiHelper.requestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                ignoreOrientation = ActivityInfo.SCREEN_ORIENTATION_PORTRAIT;
            }
...
//                    must put the action to queue so the beginDelayedTransition can take effect
    ...
    private ViewGroup createFloatBox() {
        ...
        FrameLayout.LayoutParams floatBoxParams = new FrameLayout.LayoutParams(com.grovertb.player.VideoInfo.floatView_width, com.grovertb.player.VideoInfo.floatView_height);
        if (com.grovertb.player.VideoInfo.floatView_x == Integer.MAX_VALUE || com.grovertb.player.VideoInfo.floatView_y == Integer.MAX_VALUE) {
            ...
            floatBoxParams.leftMargin = (int) com.grovertb.player.VideoInfo.floatView_x;
            floatBoxParams.topMargin = (int) com.grovertb.player.VideoInfo.floatView_y;
        }
...
//                            x = 0;
    ...
    private void removeFloatContainer() {
        ...
        if (activity != null) {
            ...
            if (floatBox != null) {
                com.grovertb.player.VideoInfo.floatView_x = floatBox.getX();
                com.grovertb.player.VideoInfo.floatView_y = floatBox.getY();
            }
    ...
    public com.grovertb.player.VideoInfo getVideoInfo() {
        return videoInfo;
    }
