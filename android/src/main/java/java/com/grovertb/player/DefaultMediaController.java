...
public class DefaultMediaController extends BaseMediaController {
    ...
    private int displayModel = com.grovertb.player.GiraffePlayer.DISPLAY_NORMAL;
    ...
    protected final SeekBar.OnSeekBarChangeListener seekListener = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            ...
            com.grovertb.player.GiraffePlayer player = videoView.getPlayer();
    ...
    protected long setProgress() {
        ...
        if (currentState == com.grovertb.player.GiraffePlayer.STATE_IDLE ||
                currentState == com.grovertb.player.GiraffePlayer.STATE_PREPARING ||
                currentState == com.grovertb.player.GiraffePlayer.STATE_ERROR) {
            return 0;
        }
    ...
    protected void show(int timeout) {
        if (!isShowing) {
            if (videoView.getVideoInfo().isShowTopBar() || displayModel == com.grovertb.player.GiraffePlayer.DISPLAY_FULL_WINDOW) {
    ...
    protected void showBottomControl(boolean show) {
        if (displayModel == com.grovertb.player.GiraffePlayer.DISPLAY_FLOAT) {
            show = false;
        }
...
//                doPauseResume();
                ...
                player.stop();
                player.setDisplayModel(com.grovertb.player.GiraffePlayer.DISPLAY_NORMAL);
            } else if (v.getId() == R.id.app_video_float_full) {
                player.setDisplayModel(com.grovertb.player.GiraffePlayer.DISPLAY_FULL_WINDOW);
...
//                        play(url);
    ...
    public void onRelease(com.grovertb.player.GiraffePlayer giraffePlayer) {
        ...
        if (videoInfo.isCurrentVideoAsCover()) {
            if (giraffePlayer.getCurrentState() != com.grovertb.player.GiraffePlayer.STATE_ERROR) {
                ...
                if (currentDisplay != null) {
                    ...
                    if (imageView != null) {
                        ...
                        if (aspectRatio == com.grovertb.player.VideoInfo.AR_ASPECT_FILL_PARENT) {
                            imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
                        } else if (aspectRatio == com.grovertb.player.VideoInfo.AR_MATCH_PARENT) {
...
//            Toast.makeText(context, "onDoubleTap", Toast.LENGTH_SHORT).show();
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            if (toSeek) {
                ...
                if (displayModel == com.grovertb.player.GiraffePlayer.DISPLAY_NORMAL && videoView.inListView()) {
                    return true;
                }
...
//                        Toaster.show("download rate:" + extra);
    ...
    public void onCurrentStateChange(int oldState, int newState) {
        if (context instanceof Activity) {
            if (newState == com.grovertb.player.GiraffePlayer.STATE_LAZYLOADING) {
            ...
            if (newState == com.grovertb.player.GiraffePlayer.STATE_PLAYING) {
                //set SCREEN_ON
...
//        }
    ...
    public void onTargetStateChange(int oldState, int newState) {
        if (newState != com.grovertb.player.GiraffePlayer.STATE_IDLE) {
